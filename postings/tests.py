...
class JobPostingViewTest(TestCase):
    ...
    def test_success_patch_job_posting(self):
        ...
        response = client.patch(
            "/postings/1", json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class CreateJobPostingTest(TestCase):
    ...
    def test_success_create_job_posting(self):
        ...
        response = client.post(
            "/postings", json.dumps(data), content_type="application/json"
        )
...
class UserJobApplyTest(TestCase):
    ...
    def test_success_user_job_apply(self):
        client = Client()
        data = {"user_id": 1, "posting_id": 2}
        response = client.post(
            "/postings/2/apply", json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.json(), {"message": "success"})
    ...
    def test_fail_400_user_job_apply(self):
        client = Client()
        data = {"user_id": 1, "posting_id": 1}
        response = client.post(
            "/postings/1/apply", json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
