...
class JobPostingView(View):
    def get(self, request, posting_id):
        ...
        data = {
            ...
            "posting_id_list_by_company": [
                item.id for item in posting.company.job_postings.all()
            ],
        }
...
def user_job_apply(request, posting_id):
    ...
    if UserJobApplication.objects.filter(
        user_id=user_id, job_posting_id=posting_id
    ).exists():
        return JsonResponse(
            {"message": "You have already applied for this job"}, status=400
        )

    UserJobApplication.objects.create(user_id=user_id, job_posting_id=posting_id)
    return JsonResponse({"message": "success"}, status=201)
